...
from ..schemas import FavoriteSong
...
@router.post("/upload", status_code=HTTPStatus.CREATED.value)
def upload_song(
    ...
    song_res = {"url": "http://example.com/song1"}
    thumbnail_res = {"url": "http://example.com/thumb1"}
...
def favorite_song(
    ...
    if fav_song:
        ...
        new_fav = Favorite(
            id=str(uuid.uuid4()), song_id=song.song_id, user_id=user_id
        )
        db.add(new_fav)
        db.commit()
        return {"message": True}
